...
unsigned long HashTable::getUsed() const
{
 return used;
}
...
void HashTable::TransferData(const int DocId, GlobalHashTable &GlobalHT) const
{
   ...
   GlobalHT.IncTotalDocs();  
   
   unsigned long get = getUsed();

   // Copy the contents of the hashtable  
   for ( unsigned long i=0; i < size; i++ )
   {  
      if ( (!(hashtable[i].key == "")) && (hashtable[i].data < 10))
         GlobalHT.Insert(hashtable[i].key, DocId, hashtable[i].data, get);
   }
}
