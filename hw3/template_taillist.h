...
// list operations
   ...
   bool IsEmpty() const;
   void Print() const;
   void Print(ofstream &Dout, float idf) const;
...
//-----------------------------------------------------------------
template <class T>
void List<T>::Print(ofstream &Dout, float idf) const
{
NodePtr Temp = Head;
int temp = 0;
   while(Temp != NULL)
...
Temp = Head;
   ...
   while (Temp != NULL)
   {
      ...
      (Temp->Item).Print(Dout, idf, temp, (Temp->Item).GetNumTokens());
      Temp = Temp->Next;
   }
