...
template <class T>
void List<T>::PrintPost() 
{

NodePtr Temp = Head;

   ofstream dout;
   dout.open("post.txt", fstream::app);

   // loop through whole list printing nodes
   while (Temp != NULL)
   {
      //Modified to get the doc id and the term frequency
       //This works by opening a file that gets appended to it every time
        //there is a key that contains a doc id and term frequenccy.

      dout << Temp->Item.doc_ID << " " << Temp->Item.Term << endl;
      Temp = Temp->Next;
   }
...
//I listened to Oh, it is Love by HelloGoodbye
// If you like mid 2000s geek rock, it's a pretty rocking song
